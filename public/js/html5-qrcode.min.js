GridSampler = {
  ...
  sampleGrid3: function (d, a, c) {
    for (var b = new BitMatrix(a), e = Array(a << 1), g = 0; g < a; g++) {
      for (var f = e.length, h = g + 0.5, j = 0; j < f; j += 2) e[j] = (j >> 1) + 0.5,
      e[j + 1] = h;
...
function Version(d, a, c, b, e, g) {
  ...
  this.buildFunctionPattern = function () {
    ...
    6 < this.versionNumber && (b.setRegion(e - 11, 0, 3, 6), b.setRegion(0, e - 11, 6, 3));
    return b
  };
...
function buildVersions() {
  ...
  ], new ECBlocks(7, new ECB(1, 19)), new ECBlocks(10, new ECB(1, 16)), new ECBlocks(13, new ECB(1, 13)), new ECBlocks(17, new ECB(1, 9))),
  new Version(2, [
    6,
    18
  ], new ECBlocks(10, new ECB(1, 34)), new ECBlocks(16, new ECB(1, 28)), new ECBlocks(22, new ECB(1, 22)), new ECBlocks(28, new ECB(1, 16))),
  new Version(3, [
    6,
    22
  ], new ECBlocks(15, new ECB(1, 55)), new ECBlocks(26, new ECB(1, 44)), new ECBlocks(18, new ECB(2, 17)), new ECBlocks(22, new ECB(2, 13))),
  new Version(4, [
    6,
    26
  ], new ECBlocks(20, new ECB(1, 80)), new ECBlocks(18, new ECB(2, 32)), new ECBlocks(26, new ECB(2, 24)), new ECBlocks(16, new ECB(4, 9))),
  new Version(5, [
    6,
    30
  ], new ECBlocks(26, new ECB(1, 108)), new ECBlocks(24, new ECB(2, 43)), new ECBlocks(18, new ECB(2, 15), new ECB(2, 16)), new ECBlocks(22, new ECB(2, 11), new ECB(2, 12))),
  new Version(6, [
    6,
    34
  ], new ECBlocks(18, new ECB(2, 68)), new ECBlocks(16, new ECB(4, 27)), new ECBlocks(24, new ECB(4, 19)), new ECBlocks(28, new ECB(4, 15))),
  new Version(7, [
    6,
    22,
    38
...
function Detector(d) {
  ...
  this.processFinderPatternInfo = function (a) {
    ...
    return new DetectorResult(d, null == j ? [
      a,
      c,
      b
    ] : [
...
function GF256(d) {
  ...
  for (var a = 1, c = 0; 256 > c; c++) this.expTable[c] = a,
  a <<= 1,
  256 <= a && (a ^= d);
...
function FinderPattern(d, a, c) {
  ...
  this.aboutEquals = function (b, a, c) {
    return Math.abs(a - this.y) <= b && Math.abs(c - this.x) <= b ? (b = Math.abs(b - this.estimatedModuleSize), 1 >= b || 1 >= b / this.estimatedModuleSize)  : !1
  }
}
...
function FinderPatternFinder() {
  ...
  this.crossCheckVertical = function (d, a, c, b) {
    for (var e = this.image, g = qrcode.height, f = this.CrossCheckStateCount, h = d; 0 <= h && e[a + h * qrcode.width]; ) f[2]++,
    h--;
    if (0 >
    h) return NaN;
    for (; 0 <= h && !e[a + h * qrcode.width] && f[1] <= c; ) f[1]++,
    h--;
    if (0 > h || f[1] > c) return NaN;
    for (; 0 <= h && e[a + h * qrcode.width] && f[0] <= c; ) f[0]++,
    h--;
    if (f[0] > c) return NaN;
    for (h = d + 1; h < g && e[a + h * qrcode.width]; ) f[2]++,
    h++;
    if (h == g) return NaN;
    for (; h < g && !e[a + h * qrcode.width] && f[3] < c; ) f[3]++,
    h++;
    if (h == g || f[3] >= c) return NaN;
    for (; h < g && e[a + h * qrcode.width] && f[4] < c; ) f[4]++,
    h++;
  ...
  function (d, a, c, b) {
    for (var e = this.image, g = qrcode.width, f = this.CrossCheckStateCount, h = d; 0 <= h && e[h + a * qrcode.width]; ) f[2]++,
    h--;
    if (0 > h) return NaN;
    for (; 0 <= h && !e[h + a * qrcode.width] && f[1] <= c; ) f[1]++,
    h--;
    if (0 > h || f[1] > c) return NaN;
    for (; 0 <= h && e[h + a * qrcode.width] && f[0] <= c; ) f[0]++,
    h--;
    if (f[0] > c) return NaN;
    for (h = d + 1; h < g && e[h + a * qrcode.width]; ) f[2]++,
    h++;
    if (h == g) return NaN;
    for (; h < g && !e[h + a * qrcode.width] && f[3] < c; ) f[3]++,
    h++;
    if (h == g || f[3] >= c) return NaN;
    for (; h < g && e[h + a * qrcode.width] && f[4] < c; ) f[4]++,
    h++;
    return f[4] >=
  ...
  this.handlePossibleCenter = function (d, a, c) {
    ...
    if (!isNaN(a) && (c = this.crossCheckHorizontal(Math.floor(c), Math.floor(a), d[2], b), !isNaN(c))) {
      ...
      b || (c = new FinderPattern(c, a, d), this.possibleCenters.push(c), null != this.resultPointCallback && this.resultPointCallback.foundPossibleResultPoint(c));
      return !0
    }
    return !1
  };
  this.selectBestPatterns = function () {
    ...
    if (3 < d) {
      ...
      d = a / d;
      for (c = 0; c < this.possibleCenters.length && 3 < this.possibleCenters.length; c++) Math.abs(this.possibleCenters[c].EstimatedModuleSize - d) > 0.2 * d && (this.possibleCenters.remove(c), c--)
    }
  ...
  this.findRowSkip = function () {
    ...
    for (var a = null, c = 0; c < d; c++) {
      ...
      if (b.Count >= CENTER_QUORUM) if (null == a) a = b;
       else return this.hasSkipped = !0,
  ...
  this.haveMultiplyConfirmedCenters = function () {
    for (var d = 0, a = 0, c = this.possibleCenters.length, b = 0; b < c; b++) {
      ...
      e.Count >= CENTER_QUORUM && (d++, a += e.EstimatedModuleSize)
    }
    if (3 > d) return !1;
    for (var d = a / c, g = 0, b = 0; b < c; b++) e = this.possibleCenters[b],
  ...
  this.findFinderPattern = function (d) {
    ...
    for (var e = !1, g = Array(5), f = b - 1; f < a && !e; f += b) {
      ...
      g[2] = 0;
      g[3] = 0;
      for (var h = g[4] = 0, j = 0; j < c; j++) if (d[j + f * qrcode.width]) 1 == (h & 1) && h++,
      g[h]++;
       else if (0 == (h & 1)) if (4 == h) if (this.foundPatternCross(g)) {
        if (h =
        this.handlePossibleCenter(g, f, j)) b = 2,
        this.hasSkipped ? e = this.haveMultiplyConfirmedCenters()  : (h = this.findRowSkip(), h > g[2] && (f += h - g[2] - b, j = c - 1));
         else {
          do j++;
          while (j < c && !d[j + f * qrcode.width]);
          j--
        }
        h = 0;
        g[0] = 0;
...
function AlignmentPatternFinder(d, a, c, b, e, g, f) {
  ...
  this.crossCheckVertical = function (b, a, c, d) {
    ...
    f[1] = 0;
    f[2] = 0;
    for (var m = b; 0 <= m && e[a + m * qrcode.width] && f[1] <= c; ) f[1]++,
    m--;
    if (0 > m || f[1] > c) return NaN;
    for (; 0 <= m && !e[a + m * qrcode.width] && f[0] <= c; ) f[0]++,
    m--;
    if (f[0] > c) return NaN;
    for (m = b + 1; m < g && e[a + m * qrcode.width] && f[1] <= c; ) f[1]++,
    m++;
    if (m == g || f[1] > c) return NaN;
    for (; m < g && !e[a + m * qrcode.width] && f[2] <= c; ) f[2]++,
    m++;
  ...
  this.find = function () {
    for (var a = this.startX, e = this.height, g = a + b, f = c + (e >> 1), l = [
      0,
      0,
      0
    ], s = 0; s < e; s++) {
      ...
      for (var q = 0; m < g; ) {
        if (d[m + r * qrcode.width]) if (1 == q) l[q]++;
         else if (2 == q) {
...
function QRCodeDataBlockReader(d, a, c) {
  ...
  this.getFigureString = function (b) {
    var a = 0,
    c = '';
    do 3 <= b ? (a = this.getNextBits(10), 100 > a && (c += '0'), 10 > a && (c += '0'), b -= 3)  : 2 == b ? (a = this.getNextBits(7), 10 > a && (c += '0'), b -= 2)  : 1 == b && (a = this.getNextBits(4), b -= 1),
    c += a;
    while (0 < b);
    return c
  };
